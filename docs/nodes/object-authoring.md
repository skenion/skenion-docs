...
Runtime may preserve the typed `objectSpec` for editing, issues, and
...
Runtime returns the accepted node/interface or a issue result. Studio
...
`objectResolution` state with issues. If more than one provider matches the
...
nodes, annotation nodes, issue placeholders, and future package-provided
